...
file_name = "data/AVAXUSDT-1m-2021-09.csv"
...
for month in range(1, 10):
    ...
    # alter month of data file
    file_name = list(file_name)
    file_name[-5] = str(month)
    file_name = "".join(file_name)

    # loop for multiple minute wait times
    for minute_wait_time in range(5, 61):
        ...
        with open(file_name, 'r') as f:
            ...
            while True:
                ...
                if counter > 0:
                    ...
                    if counter == 0:
                        if not is_buying:
                            base_amount, quote_amount = sell(base_amount, my_list[0].open,
                                                             my_list[0].open_time, transactions)
                            is_buying = True
                        else:
                            base_amount, quote_amount = buy(quote_amount, my_list[0].open,
                                                            my_list[0].open_time, transactions)
                            is_buying = False
            ...
            # record the final base_amount in the transaction list as the final result
            with open('results.txt', 'a') as results_file:
                results_file.write(f'{file_name} + {minute_wait_time} = {transactions[-1].base_amount}\n')
